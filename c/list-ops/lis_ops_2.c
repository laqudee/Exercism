...
list_t * new_list(size_t length, list_element_t values[]) {
  list_t *list = malloc(sizeof(*list) + sizeof(list_element_t) * length);
  if (!list) return NULL;
  list->length = length;
  if (values) memcpy(list->elements, values, sizeof(*values) * length);
...
list_t * filter_list(list_t * list, bool(*filter) (list_element_t value)) {
  ...
  for(size_t i=0; i< list->length; i++) {
    ...
    if (filter(v)) filtered->elements[filtered->length++] = v;
  }
...
list_element_t foldl_list(list_t * list, list_element_t initial, list_element_t(*foldl) (list_element_t value, list_element_t initial)) {
...
list_element_t foldr_list(list_t * list, list_element_t initial, list_element_t(*foldr) (list_element_t value, list_element_t initial)) {
  if (!list) return initial;
...
static void swap(list_element_t *left, list_element_t *right) {
  list_element_t temp = *left;
  *left = *right;
  *right = temp;
